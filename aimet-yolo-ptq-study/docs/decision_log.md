...
| AdaRound full 실행 방식 | `adaround-samples 256`, `iterations 5000` full run은 detached runner로만 실행 | 2026-06-28 foreground 실행은 `121/406` 모듈, 약 30%, 경과 52분 지점에서 산출물 없이 끊겼습니다. full AdaRound는 수 시간 단위 작업이고 export 전 중단되면 결과가 남지 않으므로 `scripts/13_run_adaround_full_detached.sh`로 로그와 PID를 남겨 실행합니다. |
