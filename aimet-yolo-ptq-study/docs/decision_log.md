...
| Head group selective A16 | `scripts/19_activation_encoding_intervention.py`에 head group A16 preset을 추가해 branch/scale/final/head 전체를 sample500으로 비교 | `head_cv3_outputs_a16`이 0.4072(+0.0060)로 가장 높고, `head_scale2_outputs_a16`과 `head_conv_outputs_a16`은 각각 약 +0.0042에 그쳤습니다. 따라서 더 넓은 head 전체 A16보다 `cv3` class branch group이 우선 mixed precision 후보입니다. |
...
| AdaRound full 설정 | `calib256`, `adaround-samples 256`, `iterations 5000`, `sample500` full 설정을 detached tmux runner로 완료 | mAP50-95는 0.4026으로 A8W8 QuantSim 0.4012보다 +0.0014, 중간 AdaRound 0.4036보다 -0.0011입니다. Coverage는 Q/DQ 397/397, Conv weight QDQ 102/102, Conv weight INT storage 0/102, effective INT storage 0.0%로 기존 A8W8 QDQ와 같습니다. 설정 규모를 키워도 weight rounding 단독 회복폭이 작아 activation QDQ/range가 다음 병목이라고 판단합니다. |
| AdaRound full 실행 방식 | `adaround-samples 256`, `iterations 5000` full run은 detached runner로만 실행 | 2026-06-28 foreground 실행은 `121/406` 모듈, 약 30%, 경과 52분 지점에서 산출물 없이 끊겼습니다. 2026-07-05 detached tmux 실행은 `406/406` 최적화, QDQ export, sample500 eval까지 완료했습니다. full AdaRound는 수 시간 단위 작업이고 export 전 중단되면 결과가 남지 않으므로 `scripts/13_run_adaround_full_detached.sh`와 `scripts/14_adaround_full_status.sh`를 사용합니다. |
| ORT QOperator 배포 후보 | Conv-only QOperator INT8을 별도 probe로 평가하고 AIMET QDQ 정확도 분석과 분리 | QLinearConv 102개, Conv weight INT storage 102/102, 모델 크기 2.757MB로 packed artifact는 만들었습니다. 하지만 sample500 mAP50-95는 0.3486으로 AIMET A8W8 QDQ 0.4012보다 낮고, ORT CUDA model-only latency는 32.40ms로 FP32 6.16ms와 A8W8 QDQ 14.77ms보다 느렸습니다. 따라서 ORT CUDA 기준 추천 배포 후보가 아니며, TensorRT/QNN/EP 친화 export는 별도 검증해야 합니다. |
| TensorRT EP preflight | latency 스크립트에 provider 선택과 fallback guard를 추가하고 TensorRT EP 로드를 확인 | ONNX Runtime provider 목록에는 `TensorrtExecutionProvider`가 보이지만, 실제 로드는 `libnvinfer.so.10` 누락으로 실패했습니다. 스크립트는 TensorRT가 CUDA로 fallback된 값을 TensorRT latency로 기록하지 않도록 요청 provider가 활성화되지 않으면 실패시킵니다. 현재 환경에서는 TensorRT runtime 설치 후 다시 측정해야 합니다. |
