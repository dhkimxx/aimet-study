...
상태: 논문형 리포트 초안. 현재 수치는 `sample100` 탐색 결과와 `sample500` 확대 검증 결과를 포함합니다. 최종 결론 전에는 full COCO val 평가가 필요합니다.

## 초록

이 연구는 YOLO26n ONNX detection 모델을 대상으로 AIMET ONNX 2.2.0 기반 post-training quantization(PTQ)을 WSL2 Ubuntu native GPU 환경에서 재현 가능하게 비교한다. 단순 ONNX Runtime INT8 양자화, AIMET QuantSim, CLE, AdaRound, 8/16비트 조합, activation QDQ ablation을 같은 평가 파이프라인으로 비교했다. 핵심 발견은 세 가지다. 첫째, naive ONNX INT8은 postprocess와 graph output까지 공격적으로 양자화하면서 sample100과 sample500 모두에서 mAP가 0으로 붕괴했다. 둘째, AIMET QDQ는 sample500에서 FP32 0.4203 대비 A8W8 0.4012로 정확도를 유지하지만 현재 산출물은 Conv weight storage가 FP32인 accuracy-eval QDQ 모델이므로 배포 효율과 분리해서 해석해야 한다. 셋째, 16비트 조합과 activation-QDQ 제거 ablation은 A8W8 손실의 주된 원인이 weight보다 activation quantization error임을 보여준다.

## 연구 질문

| ID | 질문 | 현재 답 |
| --- | --- | --- |
| RQ1 | no-AIMET naive INT8은 YOLO ONNX에 유효한 기준선인가? | 정확도 기준으로는 실패 기준선입니다. sample100과 sample500 모두 mAP50-95가 0으로 붕괴했습니다. |
| RQ2 | AIMET QDQ PTQ는 naive INT8 대비 정확도를 유지하는가? | 예. A8W8 QuantSim은 sample500에서 mAP50-95 0.4012를 유지했습니다. |
| RQ3 | A8W8 손실은 weight와 activation 중 어디에 더 민감한가? | activation이 더 민감합니다. sample500에서 A16W8이 A8W16보다 높고, sample100 all-activation-float ablation이 FP32 수준까지 회복했습니다. |
...
| Dataset | COCO 2017 val subset, quick result는 100 images, expanded result는 500 images |
...
| 실험 | Activation | Weight | Opset | sample100 mAP50-95 | sample500 mAP50-95 | sample500 FP32 대비 | Conv weight INT storage |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| QuantSim | 8 | 8 | 17 | 0.5174 | 0.4012 | -0.0191 | 0/102 |
| QuantSim | 16 | 8 | 21 | 0.5449 | 0.4143 | -0.0060 | 0/102 |
| QuantSim | 8 | 16 | 21 | 0.5347 | 0.4074 | -0.0130 | 0/102 |
| QuantSim | 16 | 16 | 21 | 0.5374 | 0.4170 | -0.0033 | 0/102 |

A16W8이 A8W16보다 더 크게 회복했다. 이는 activation quantization error가 weight quantization error보다 더 지배적일 가능성을 보인다. A16W16은 sample500 정확도만 보면 가장 좋지만, 네 모델 모두 Conv weight QDQ는 102/102이고 Conv weight INT storage는 0/102라 파일 내부 weight가 packed int8/int16으로 저장된 배포 산출물이 아니다.
...
1. 현재 확대 검증도 500장 subset 기준이라 full COCO val 통계는 아직 없다.
...
| 우선순위 | 작업 | 완료 기준 |
| --- | --- | --- |
| P0 | full COCO val로 주요 정확도 재평가 | FP32, naive INT8, A8W8, 16비트 후보, AdaRound 정식 결과표 |
