...
상태: 논문형 리포트 초안. 현재 수치는 `sample100` 탐색, `sample500` 확대 검증, full COCO val 5천 장 핵심 결과를 포함합니다. 남은 주요 공백은 AdaRound 정식 설정과 배포형 export입니다.

## 초록

이 연구는 YOLO26n ONNX detection 모델을 대상으로 AIMET ONNX 2.2.0 기반 post-training quantization(PTQ)을 WSL2 Ubuntu native GPU 환경에서 재현 가능하게 비교한다. 단순 ONNX Runtime INT8 양자화, AIMET QuantSim, CLE, AdaRound, 8/16비트 조합, activation QDQ ablation을 같은 평가 파이프라인으로 비교했다. 핵심 발견은 세 가지다. 첫째, naive ONNX INT8은 postprocess와 graph output까지 공격적으로 양자화하면서 sample100, sample500, full COCO val 모두에서 mAP가 0으로 붕괴했다. 둘째, AIMET A8W8 QDQ는 full COCO에서 FP32 0.3971 대비 QuantSim calib64 0.3740, QuantSim calib1024 0.3787, CLE calib1024 0.3788 mAP50-95로 정확도를 유지하지만, 현재 산출물은 Conv weight storage가 FP32인 accuracy-eval QDQ 모델이므로 배포 효율과 분리해서 해석해야 한다. 셋째, full COCO 16비트 조합과 activation-QDQ 제거 ablation은 A8W8 손실의 주된 원인이 weight보다 activation quantization error임을 보여준다.
...
1. AdaRound는 아직 full COCO 정식 설정으로 재평가하지 않았다.
...
| 우선순위 | 작업 | 완료 기준 |
| --- | --- | --- |
| P0 | full COCO val로 주요 정확도 재평가 | FP32, naive INT8, A8W8, CLE calib1024, 16비트 후보 완료. AdaRound 정식은 P1로 분리 |
