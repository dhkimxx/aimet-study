...
| AIMET AdaRound + QuantSim | A8W8, calib256, adar256, iter5000, sample500 | CUDA | 0.4026 | 0.5552 | 0.4371 | 0.6452 | 0.5116 | -0.0177 |

해석: naive INT8의 mAP 0 붕괴는 sample500에서도 그대로 재현됩니다. A8W8 QDQ는 FP32 대비 -0.0191 mAP50-95로 정확도를 유지하지만 완전한 복원은 아닙니다. AdaRound 중간 설정은 A8W8 QuantSim보다 +0.0025 높았고, 더 긴 full 설정(`adaround-samples 256`, `iterations 5000`)은 +0.0014만 높아 중간 설정보다 오히려 -0.0011 낮았습니다. 둘 다 A8W16 0.4074, A16W8 0.4143, A16W16 0.4170에는 못 미칩니다. 따라서 weight rounding 최적화만으로는 현재 손실 대부분을 설명하기 어렵습니다. 16비트 조합은 A16W16이 FP32에 가장 가깝고, 단일 축 비교에서는 A16W8이 A8W16보다 더 크게 회복합니다. 따라서 sample500에서도 activation bitwidth를 올리는 효과가 weight bitwidth를 올리는 효과보다 큽니다.
...
| AIMET CLE + QuantSim | A8W8, calib1024, full val | CUDA | 0.3788 | 0.5347 | 0.4127 | 0.6448 | 0.4962 | -0.0183 |
...
해석: full COCO에서도 sample500 결론이 유지됩니다. naive INT8은 완전히 붕괴했고, A8W8 QDQ는 정확도를 유지하되 FP32 대비 -0.0231 mAP50-95 손실이 남았습니다. Calibration sample을 64에서 1024로 늘린 A8W8은 0.3787로 +0.0047만 회복했습니다. CLE calib1024는 0.3788로 QuantSim calib1024와 사실상 같고, 이 ONNX에는 BatchNorm이 없어 AIMET high-bias folding이 지원되지 않았습니다. A16W8이 A8W16보다 높아 activation 쪽 민감도가 더 크고, A16W16은 FP32에 거의 붙지만 ORT CUDA latency가 100ms 이상이라 배포 후보가 아니라 정확도 상한으로 해석합니다.
...
| AdaRound A8W8 calib256 adar256 iter5000 | 17 | 397/397 | 102/102 | 0/102 | 102 | 295 | 0 | 0 | 9.786 |
...
| 실험 | 설정 | Runtime | mAP50-95 | mAP50 | mAP75 | Precision | Recall | FP32 대비 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| ORT QOperator INT8 | Conv-only, calib64, sample500 | CUDA | 0.3486 | 0.4948 | 0.3771 | 0.6083 | 0.4510 | -0.0717 |

커버리지와 latency:

| 모델 | Q/DQ | QLinearConv | Conv weight INT storage | Effective Conv INT storage | 모델 크기 MB | model-only 평균 ms | end-to-end 평균 ms | ORT Memcpy |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| ORT QOperator Conv-only | 92/102 | 102 | 102/102 | 100% | 2.757 | 32.40 | 43.97 | 194 |

해석: QOperator Conv-only 모델은 graph output과 postprocess를 float로 남겨 naive INT8처럼 mAP 0으로 붕괴하지는 않았고, Conv weight도 실제 int8 initializer로 저장했습니다. 그러나 sample500 mAP50-95가 0.3486으로 AIMET A8W8 QDQ 0.4012보다 낮고, ORT CUDA model-only latency도 32.40ms로 FP32 6.16ms 및 AIMET A8W8 QDQ 14.77ms보다 느렸습니다. 따라서 이 산출물은 "packed storage는 가능하다"는 근거지만, 현재 RTX 3070 ORT CUDA 배포 후보로는 부적합합니다.
...
| E256 | `6e5ce32b72b37889f0d4092db53237b6ba2a3c0635fedf86e8753760ee80b83e` | AIMET AdaRound calib256 adar256 iter5000 QDQ |
| G | `6ba6f634b561936e310c483b67c558ac982cd8dd77090c2393cb5d86f7fd21ea` | ORT QOperator Conv-only INT8 |
